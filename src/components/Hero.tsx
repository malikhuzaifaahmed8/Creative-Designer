...
const Hero = () => {
  ...
  useEffect(() => {
    ...
    const firstSpanSetters = Array.from(firstSpans as HTMLSpanElement[]).map(
      (span) =>
    ...
    const lastSpanSetters = Array.from(lastSpans as HTMLSpanElement[]).map(
      (span) =>
        gsap.quickTo(span, "scaleX", {
          duration: 0.5,
          ease: "power2.out",
    ...
    const handleMouseMove = (e: MouseEvent) => {
      ...
      const scaleMapped = gsap.utils.mapRange(
        0,
        centerX,
        0.8,
        1.2,
      ...
      lastSpanSetters.forEach((setScaleX: gsap.QuickToFunc, index: number) => {
        const scaleValue =
          index % 2 === 0
            ? isLeft
              ? scaleMapped
  ...
  return (
    ...
    >
      ...
      >
        <Image
          src="/designer-portfolio.jpg" // Changed to generic designer image
          alt="Creative Designer portfolio showcasing design work"
          width={300}
          height={300}
